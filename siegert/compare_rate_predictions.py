...
C = 250.0
J = 600.0
# scale connection strength by tau_syn such that results for different synaptic time constants are comparable
PSC = J/tau_syn

neurondict = {'V_th': V_th, 'tau_m': 10.0, 'E_L': V0, 't_ref': taur,
              'V_reset': V0, 'C_m': C, 'tau_syn_ex': tau_syn}
neurondict2 = {'V_th': V_th2, 'tau_m': 10.0, 'E_L': V02, 't_ref': taur,
               'V_reset': V0, 'C_m': C, 'tau_syn_ex': tau_syn}
nest.SetDefaults('iaf_psc_exp', neurondict)
nest.SetDefaults('gif2_psc_exp', neurondict2)
neuron = nest.Create("iaf_psc_exp", N)
neuron2 = nest.Create("gif2_psc_exp", N)
...
nest.Connect(noise, neuron2, 'all_to_all', syn_spec=syn_dict)
...
nest.Connect(neuron2, spikedetector2, 'all_to_all')

nest.Simulate(T)

# get firing rate of neurons
rate = nest.GetStatus(spikedetector, "n_events")[0]
rate2 = nest.GetStatus(spikedetector2, "n_events")[0]
