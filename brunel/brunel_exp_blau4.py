"""
Script to run a brunel network representing a population from area V1, L5.
Resonating neurons are represented by the GIF2 model from Richardson et al.
(2003) and are guessed to make up 50% of the excitatory popoulation size
Silva et al. (1979).

The script uses mingtools1 (v1.0) and is supposed to be started with a shell
script to be run on a cluster. NEST v2.12 required and the gif2_module.

Daniel Mingers, Sep 2016

v4.0: adapted the conn_rule to fixed_total_number as in Potjans and Diesmann
 (2012).
"""
...
parspace = np.arange(100., 600, 16, dtype=float)
#C_m2 = 250.0
C_m2 = parspace[ int(sys.argv[ 2 ]) ]  # 250.0
g_1 = 35.0  # 85.0
g_m = 25.0  # 35.0
theta2 = 20.0
neuron_params2 = {
...
h = 5.0  # 1.625  # ratio resonating weight/excitatory weight
J_re = h * J
...
if alternative_connection:
    ...
    nest.Connect(nodes_ex, nodes_ex,
                 conn_spec= {'rule': 'fixed_total_number', 'N': C[ 0, 0 ]},
                 syn_spec={'weight': J[ 0, 0 ]})
    nest.Connect(nodes_ex, nodes_re,
                 conn_spec={'rule': 'fixed_total_number', 'N': C[ 0, 1 ]},
                 syn_spec={'weight': J[ 0, 1 ]})
    nest.Connect(nodes_ex, nodes_in,
                 conn_spec={'rule': 'fixed_total_number', 'N': C[ 0, 2 ]},
    ...
    nest.Connect(nodes_re, nodes_ex,
                 conn_spec={'rule': 'fixed_total_number', 'N': C[ 1, 0 ]},
                 syn_spec={'weight': J[ 1, 0 ]})
    nest.Connect(nodes_re, nodes_re,
                 conn_spec={'rule': 'fixed_total_number', 'N': C[ 1, 1 ]},
                 syn_spec={'weight': J[ 1, 1 ]})
    nest.Connect(nodes_re, nodes_in,
                 conn_spec={'rule': 'fixed_total_number', 'N': C[ 1, 2 ]},
    ...
    nest.Connect(nodes_in, nodes_ex,
                 conn_spec={'rule': 'fixed_total_number', 'N': C[ 2, 0 ]},
                 syn_spec={'weight': J[ 2, 0 ]})
    nest.Connect(nodes_in, nodes_re,
                 conn_spec={'rule': 'fixed_total_number', 'N': C[ 2, 1 ]},
                 syn_spec={'weight': J[ 2, 1 ]})
    nest.Connect(nodes_in, nodes_in,
                 conn_spec={'rule': 'fixed_total_number', 'N': C[ 2, 2 ]},
