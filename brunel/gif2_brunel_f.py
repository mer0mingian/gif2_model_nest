...
def run_brunel(networkparamdict, fraction):
    ...
    for spiketrain in spiketrains_all:
        if np.any(spiketrain):
            ...
            plt.plot(spiketrain, j*np.ones_like(spiketrain), 'k.', markersize=5)
            plt.xlim(0, simtime)
...
if __name__ == '__main__':
    ...
    networkparamdict = {'p_rate': 65000.0, 'C_m': 500.0, 'g': 25.0,
                        'g_1': 25.0, 'tau_1': 100.0, 'V_dist': 6.0}
    #networkparamdict = {'p_rate': 65000.0, 'C_m': 200.0, 'g': 46.0,
    #                    'g_1': 46.0, 'tau_1': 100.0, 'V_dist': 5.615}

    fractionindex = float(sys.argv[ 1 ])
